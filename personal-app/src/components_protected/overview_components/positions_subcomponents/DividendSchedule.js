...
export default function DividendSchedule() {
  ...
  const { months, ytdLabel, rows } = useMemo(() => {
    ...
    const builtRows = Object.keys(dividendByTicker).map((ticker) => {
      const monthValues = {};
      let total = 0;
      let ytd = 0;
  ...
  const renderSortControls = (key) => {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
        <button onClick={() => onSortClick(key, "desc")} title="Sort Descending" style={{ width: 14, height: 14, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#08519c", color: rule?.direction === "desc" ? "#fb6a4a" : "#f7fbff", fontSize: 10, padding: 0, lineHeight: 1 }}>▼</button>
        <button onClick={() => onSortClick(key, "asc")} title="Sort Ascending" style={{ width: 14, height: 14, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#08519c", color: rule?.direction === "asc" ? "#fb6a4a" : "#f7fbff", fontSize: 10, padding: 0, lineHeight: 1 }}>▲</button>
        <button onClick={() => onSortClick(key, "remove")} title="Remove Sort" style={{ width: 14, height: 14, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#08519c", color: "#f7fbff", fontSize: 10, padding: 0, lineHeight: 1 }}>✕</button>
      </div>
    );
  };
