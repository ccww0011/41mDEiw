...
export default function WalkthroughStock() {
  ...
  const {
    ...
    setPrices,
    loadingPrices,
    setLoadingPrices} = useWalkthroughContext();
  ...
  return (
    <>
      ...
      {!filteredPrices || Object.keys(filteredPrices[selectedTicker] || {}).length === 0 ? (
        ...
        <>
          <WalkthroughGraph prices={filteredPrices} selectedItem={selectedTicker} />
          <WalkthroughTable prices={filteredPrices} selectedItem={selectedTicker} digits={2} />
        </>
      )}
    </>
  );
